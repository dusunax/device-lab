...
NTP 재동기화 없이 RTC 자체 크리스탈만으로 시간이 유지되는지 확인합니다. 부팅 직후(WiFi 연결
전, NTP가 RTC를 덮어쓰기 전) 찍히는 첫 `rtc_read`가 판단 기준입니다.

테스트 절차:

1. `rtc_write`가 찍힌 시점의 `details.datetime`(또는 그 직후 실제 시각)을 기록해둡니다.
2. 시간이 충분히 지난 뒤(리부팅/재업로드를 여러 번 거쳐도 무방) 리셋합니다.
3. 부팅 직후(첫) `rtc_read`의 `details.datetime`을 리셋을 실행한 실제 시각과 비교합니다.
4. `oscillator_stopped`가 계속 `false`이고, 시각이 실제 경과 시간만큼 정확히 흘러 있으면 유지
   확인으로 판정합니다.
...
| 항목 | 값 |
| --- | --- |
| 마지막 NTP 동기화 | `2026-09-17 23:38:19` |
| 경과 시간 | 약 23시간 (그 사이 여러 번 리셋/재업로드) |
| 리셋 실행 시각(호스트 PC 기준) | `2026-09-18 22:17:16` |
