...
| 🕐 RTC | 현재 시간 읽기 | PCF85063(0x51)을 Wire로 직접 구현(BCD 레지스터). NTP 동기화 성공 시 RTC에 시각 반영 후 재읽기로 확인 | ✅ |
| 🕐 RTC 유지 | 재부팅 후에도 시간 유지 | 마지막 NTP 동기화 후 약 23시간, 여러 번의 리셋/재업로드를 거친 뒤에도 NTP 재동기화 전 RTC 읽기 값이 실제 시각과 거의 정확히 일치함을 확인 | ✅ |
| 🌐 HTTP | 외부 API 요청 | HTTPClient+WiFiClientSecure(HTTPS)로 Open-Meteo API 호출, 200 응답 및 JSON 파싱 확인 | ✅ |
| ☀️ 날씨 | 실제 날씨 데이터 가져와 표시 | Open-Meteo(무료, API 키 불필요) 현재 날씨 조회. 화면 온습도 줄에 기온/날씨 텍스트(CLOUDY 등) 함께 표시 | ✅ |
| 🔘 버튼 | 버튼 입력 감지 | - | ⬜ |
