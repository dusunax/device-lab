...
## 12. `display_refresh_done` 로그는 정상인데 화면이 흰색으로 나올 때

증상: Serial telemetry에서 `display_refresh_start` → `display_refresh_done`까지 에러 없이
찍히는데, 실제 e-paper 화면은 흰색 그대로다.

원인: `EPD_1IN54G_Display()`는 SPI로 데이터를 밀어 넣기만 하고 성공/실패를 반환하지 않는다.
리본 케이블이 완전히 꽂히지 않은 상태에서도 firmware 쪽 코드는 정상 흐름대로 끝까지 실행되고
로그도 정상으로 남는다.

해결: 리본 케이블을 다시 꽂고 리셋 후 재시도한다. 실제로 케이블을 다시 꽂은 뒤 화면이 정상
출력되는 것을 확인했다.

확인 방법: 같은 조건(같은 firmware, 같은 리셋)에서 로그는 매번 정상인데 화면만 간헐적으로
흰색이면 코드 문제가 아니라 케이블 접촉 문제일 가능성이 높다.

## 13. 리셋을 반복하면 I2C scan이 매우 느려지거나 멈추는 것처럼 보일 때

증상: 짧은 간격으로 리셋을 여러 번 반복하면(특히 speaker/ES8311 관련 코드 테스트 중), 부팅 후
첫 I2C scan이 정상보다 훨씬 오래 걸리거나 `battery_adc_init`까지 도달하는 데 수십~수백 초가
걸리는 것처럼 보인 적이 있다.

원인 조사: 처음엔 ES8311이 I2C 트랜잭션 중간에 리셋되어 SDA를 계속 붙잡고 있는(bus stuck)
문제로 의심해 `recoverI2CBus()`(SDA가 LOW로 붙잡혀 있으면 SCL을 최대 9회 펄스한 뒤 수동 STOP
조건을 보내는 표준 I2C 버스 복구 절차)를 추가했다. 이후 재현 시 `i2c_bus_recovered` 이벤트가
매번 `stuck:false`로 나와, 실제 I2C 버스가 물리적으로 막힌 상태는 아니었던 것으로 보인다.
소프트/하드 리셋은 ESP32-S3만 리셋하고 외부 I2C 장치(ES8311 등)는 리셋하지 않으므로, 버스가
막힐 가능성 자체는 남아있어 이 복구 루틴은 안전장치로 유지한다.

해결: `Wire.begin()` 전에 `recoverI2CBus(sda, scl)`를 호출한다. 실제 원인은 14번 항목 참고
— 이번에 관찰된 지연은 버스 문제가 아니라 Serial 쓰기 블로킹이었을 가능성이 높다.

## 14. `firmware_version` 로그 직후부터 다음 로그까지 수초~수십 초씩 걸릴 때

증상: `firmware_version` 로그가 찍힌 뒤 다음 로그(`i2c_bus_recovered`/`i2c_scan_start` 등)가
나오기까지 정상적으로는 수 ms면 될 구간이 수초에서 최대 60초 이상 걸린 적이 있다. 매번 그
직후 로그 한 줄이 필드 일부가 사라진 채로 깨져서 출력된다.

원인 추정: `telemetryLog()`의 `Serial.print()`는 `if (!Serial) return;`로 host 연결 여부만
확인하고, USB CDC TX 버퍼가 찬 상태에서 host가 드레인하지 않으면 실제로는 블로킹될 수 있다.
`firmware_version` 로그는 상대적으로 긴 JSON 문자열이라 이 지점에서 버퍼가 찰 가능성이 높고,
그 직후의 (짧은) 다음 로그 호출이 버퍼 여유가 생길 때까지 대기하면서 uptime_ms가 크게 뛴
것으로 보인다. `screen`/`cat`로 포트를 열고 닫기를 반복하며 테스트할 때 host의 드레인 타이밍이
불안정해지는 것이 트리거로 의심된다.

범위: 이 문제는 speaker/I2C 코드와 무관한 `snow_telemetry.cpp`의 기존 로직이었다. 실제로는
로그만 늦게 찍히는 게 아니라 코드 실행 자체가 멈춰 있어서, 리셋 후 스피커 소리가 40초 이상
늦게 나는 등 사용자가 체감할 정도로 영향이 있었다.

해결: `telemetryLog()`에서 `Serial.availableForWrite()`로 TX 버퍼 여유(96바이트 미만)를 확인해,
여유가 없으면 블로킹 대신 로그를 건너뛰도록 수정했다(`feat/snow-speaker` 브랜치에 포함).

## 15. `http://snow.local/...`에 접속했는데 흰 화면만 나올 때

증상: Snow가 실제로 mDNS 응답을 시작했고(`mic_recorder_started` 로그 확인) firmware 쪽은
정상인데, 브라우저에서 `http://snow.local/...`에 접속하면 흰 화면만 뜨고 아무 내용도 안 보였다.

원인: mDNS(`.local`) 해석이 클라이언트 환경(브라우저/OS/네트워크)에 따라 실패하거나 엉뚱한
주소로 해석될 수 있다. 실제로 이 세션의 개발 환경(Snow와 다른 네트워크)에서 `snow.local`을
확인했을 때 완전히 무관한 공인 IP로 해석된 적이 있었다 — 이는 개발 환경이 Snow와 같은 네트워크가
아니어서 발생한 것으로, Snow 쪽 mDNS 응답 자체의 문제는 아니었다. 다만 사용자의 실제 브라우저
환경에서도 동일하게 `.local` 해석이 실패했다.

해결: Snow의 실제 IP로 직접 접속(`http://<IP>/...`)하면 정상 동작한다. IP는 프로젝트 원칙상
Serial/화면에 상시 노출하지 않으므로, 필요할 때만 일회성으로 `Serial.println(WiFi.localIP())`를
임시로 추가해 확인한 뒤 커밋 전에 되돌리는 방식을 사용했다.

확인 방법: mDNS 호스트네임으로 접속이 안 되면, 같은 네트워크의 다른 기기에서 그 호스트네임이
해석되는지 먼저 확인한다(`ping snow.local` 등). 완전히 다른 네트워크에서 확인하면 의미 없는
결과가 나올 수 있다.

## 16. e-paper 화면 흑백이 전체적으로 반전되어 보임 (알려진 이슈, 미해결)

증상: 코드에서는 `Paint_Clear(EPD_1IN54G_WHITE)`로 배경을 흰색으로, 텍스트는 `EPD_1IN54G_BLACK`
전경/`EPD_1IN54G_WHITE` 배경으로 그리는데, 실제 화면은 카드 배경이 검은색이고 각 상태 줄만
흰 배경 박스 안에 검은 글자로 나온다. 노란색(제목 배너)과 빨간색(날짜/SNOW READY)은 정상으로
보인다. 프로젝트 초기(Speaker/Mic 작업 이전)부터 계속 이 상태였던 것으로 확인됨.

시도한 원인 조사: `EPD_1IN54G_Display()`는 packed 픽셀 바이트를 그대로 패널에 전송하기만 하고,
실제 색상 해석은 `EPD_1IN54G_Init()`에서 설정하는 LUT/레지스터가 담당한다. 이에 따라
`EPD_1IN54G_BLACK`/`EPD_1IN54G_WHITE` 매크로 값을 전역적으로 맞바꾸는 시도를 해봤으나, 전체
캔버스 클리어(`Paint_Clear`)와 개별 텍스트 배경 채우기가 서로 다르게 반응해(캔버스는 여전히
검게, 텍스트 배경 박스만 흰색으로 반전) 단순 매크로 치환으로는 일관되게 고쳐지지 않았다.

범위: 원인이 명확하지 않고(패널 LUT vs GUI_Paint 채우기 로직 간 상호작용으로 추정) 여러 줄/여러
