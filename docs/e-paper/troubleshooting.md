...
범위: 이 문제는 speaker/I2C 코드와 무관한 `snow_telemetry.cpp`의 기존 로직이다. 지금은 현상만
기록하고, `Serial.availableForWrite()` 확인 후 버퍼 여유가 없으면 로그를 건너뛰는 방식의 수정은
별도 작업으로 분리한다.

확인 방법: Serial reader를 열고 닫기를 반복하지 않고 한 번만 연 상태로 오래 관찰하면 재현
빈도가 줄어드는 것으로 보인다(확실한 재현 조건은 아직 특정하지 못함).
